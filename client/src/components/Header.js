  ...
  return (
    <div className="border-b-2 border-black border-opacity-25">
      <div className="px-20 flex justify-between items-center pt-5 pb-3">
        <Link className="text-4xl font-bold" to={"/"}>ShopWiz</Link>
        ...
        <div className="flex items-center gap-10">
          ...
          <div>
            {isAuthenticated ? (
              <Link to={"profile"}>
                {isDataToken?.imageUrl ? (
                  <img
                    src={isDataToken.imageUrl}
                    alt={isDataToken.username}
                    className="h-10 w-10 rounded-full object-contain"
                  />
                ) : (
                  <div className="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center">
                    <span className="text-gray-600">{isDataToken?.username?.charAt(0)}</span>
                  </div>
                )}
              </Link>
            ) : (
              <Link to={"login"} className="border text-white font-semibold px-4 py-2 rounded-md bg-[#db4444] hover:bg-[#db3333]">
                Login
              </Link>
            )}
          </div>
